...
export function MotionAffectMatrix({ sessions }: { sessions: SessionSummary[] }) {
  ...
  return (
    <Card className="p-4">
      <h3 className="font-bold">{language === "zh" ? "动作和状态关系" : "Motion and state relationship"}</h3>
      <p className="mt-1 text-sm text-stone-500">
        {language === "zh" ? "看不同动作强度下，孩子更常出现哪种状态。" : "Shows which state appears more often at different motion levels."}
      </p>

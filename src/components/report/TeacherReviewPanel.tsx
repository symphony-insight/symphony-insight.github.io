  ...
  return (
    <Card className="p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h3 className="text-lg font-bold">老师看过后再导出</h3>
          <p className="mt-1 text-sm text-stone-500">报告不会自动发给家长。老师看过后，才能导出摘要。</p>
        </div>
        ...
        <div className="mt-2 space-y-2">
          {auditLogs.map((log) => (
            <div key={log.id} className="rounded-md border border-stone-200 bg-white p-3 text-sm">
              <p className="font-semibold">{actionLabels[log.action] ?? "老师更新了记录"}</p>

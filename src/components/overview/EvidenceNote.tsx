...
const references = [
  {
    ...
    en: "Joint attention and turn-taking"
  },
  {
  ...
  },
  {
    ...
    en: "Active time, return time, and teacher prompts"
  }
];
    ...
    <details className="surface group rounded-2xl p-5 shadow-card">
      <summary className="flex cursor-pointer items-center justify-between gap-3 marker:content-['']">
        <span className="flex items-center gap-2.5">
          ...
          <span>
            <span className="block text-xs font-bold uppercase tracking-[0.12em] text-tide-600">{language === "zh" ? "关于可信度" : "Evidence"}</span>
            <span className="block font-display text-base font-extrabold tracking-tightish">{language === "zh" ? "这些观察是怎么来的" : "How the evidence is built"}</span>
          </span>
        </span>
      ...
      <div className="mt-4 space-y-4 text-sm leading-6 text-ink-soft">
        <p>
          {language === "zh"
            ...
            : "Scores are built from recorded activity behaviors, then reviewed by the teacher. Each card shows the sessions and signals behind the score."}
        </p>

        <div className="rounded-xl border border-white/70 bg-paper-warm/70 p-4">
          <p className="text-xs font-bold text-ink-muted">{language === "zh" ? "9 项观察问题参考了这些方向" : "Reference areas"}</p>
        ...
        <p className="flex items-start gap-2 rounded-xl border border-moss/20 bg-moss-50 p-4 text-moss-600">
          ...
          <span className="font-medium text-ink-soft">
            {language === "zh"
              ...
              : "These are teacher-facing notes, not conclusions about the child. Read them with the live session context."}
          </span>
        </p>
        ...
        >
          {language === "zh" ? "查看完整评分说明" : "Open scoring guide"}
